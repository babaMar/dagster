...
def test_dg_init_override_workspace_name(monkeypatch) -> None:
    ...
    with ProxyRunner.test() as runner, runner.isolated_filesystem():
        result = runner.invoke("init", "--use-editable-dagster", input="my-workspace\nhelloworld\n")
        assert_runner_result(result)
        ...
        assert Path("my-workspace/projects/helloworld").exists()
        assert Path("my-workspace/projects/helloworld/helloworld").exists()
        assert Path("my-workspace/projects/helloworld/pyproject.toml").exists()
        assert Path("my-workspace/projects/helloworld/helloworld_tests").exists()
